...
export function AppleExperience() {
  ...
  return <section className={styles.section} id="intelligence" aria-labelledby="siri-title">
    <header className={styles.heading} data-reveal><p>Siri и Apple Intelligence</p><h2 id="siri-title">Спросите.<br />И продолжайте.</h2><p className={styles.intro}>Найти, сверить, написать — одним разговором.</p></header>
    ...
    <div className={styles.features}>
      <article><WandSparkles /><h3>Правьте снимок.<br />Сохраняйте замысел.</h3><p>Уберите лишнее, поправьте композицию и оставьте кадр естественным.</p><div className={styles.photo}><Image src="/media/iphone-18-pro/images/natural/landscape.webp" alt="Рисовые террасы Бали" fill sizes="(max-width:760px) 90vw,520px" /></div></article>
      <article><ShieldCheck /><h3>Ваши данные.<br />Ваши правила.</h3><p>Разрешения и доступы собраны там, где их легко проверить.</p><div className={styles.privacy}><ShieldCheck size={72} /><span>Конфиденциальность</span><p>Вы решаете, чем делиться.</p></div></article>
    </div>
    <div className={styles.one}><div><p>Apple One</p><h3>Музыка, кино,<br />игры и облако.</h3><p>Любимые сервисы собраны в одной подписке.</p><a href="https://www.apple.com/apple-one/" target="_blank" rel="noreferrer">Подробнее об Apple One ↗</a></div><div className={styles.services}>{[{ Icon: Music2, name: 'Music' }, { Icon: Tv, name: 'TV' }, { Icon: Gamepad2, name: 'Arcade' }, { Icon: Cloud, name: 'iCloud+' }].map(({ Icon, name }) => <div key={name}><Icon size={38} /><span>{name}</span></div>)}</div></div>
  </section>;
}
